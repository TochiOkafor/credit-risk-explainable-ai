...
Each decline is converted into the ranked reasons a lender is legally required to provide. Example from the final gender-free model:
...
Every reason listed is a lawful basis for a credit decision. **That was not true of the original model** — see the fairness evaluation below.
...
2. **External credit scores drive predictions** — consistent from correlation analysis through SHAP — confirming the model relies on creditworthiness, not demographics, for its core signal
...
[Click here to view the full notebook](https://nbviewer.org/github/TochiOkafor/credit-risk-explainable-ai/blob/main/notebooks/credit_risk_analysis.ipynb)
